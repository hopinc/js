...
readonly channels: {
		create<T extends State>(
			...
			Channel & {
				...
				patchState<T_2 extends State>(
					state: SetStateAction<T_2>,
				): Promise<void>;
		...
		setState<T_3 extends State = State>(
			channel: string | Channel,
			state: SetStateAction<T_3>,
		): Promise<void>;
		patchState<T_4 extends State>(
			channel: string | Channel,
			state: SetStateAction<T_4>,
		): Promise<void>;
		publishMessage<T_5>(
			channel: string | Channel,
			event: string,
		...
		tokens: {
			...
			isOnline(
				idOrToken: `leap_token_${string}` | ChannelToken,
			): Promise<boolean>;
			publishDirectMessage<T_6>(
				token: `leap_token_${string}`,
				event: string,
				data: T_6,
			): Promise<void>;
